...
#############################################################################
# PART X: Temporal Dynamics Plots ###########################################
#############################################################################
# PART X: Daytime vs Nighttime scavenging probability plot --------------------

scav_prob_AMPM_plot <- carcass_level_summary %>% 
  group_by(deployment_type_AM_PM) %>% 
  summarise(mean = mean(scav_prob),
            n = n(),
            SD = sd(scav_prob),
            SE = sd(scav_prob)/sqrt(n),
            CI = 1.96*SE)

AM_PM_plot <- ggplot()+
  geom_jitter(data=carcass_level_summary, aes(x=deployment_type_AM_PM, y=scav_prob, color=deployment_type_AM_PM), alpha = .6, height = .1, width = .2, size = 3, shape = 16)+
  geom_pointrange(data=scav_prob_AMPM_plot, aes(x=deployment_type_AM_PM, y=mean, ymin=mean-CI, ymax=mean+CI, color=deployment_type_AM_PM), alpha=0.9, size=1.3)+
  theme_few()+
  labs(x="Deployment Period", y="Probability of Carcass Scavenging")+
...
#Export high-quality figure of AM PM plot

pdf("output/supp_figures/AM_PM_plot.pdf", 
    width = 6, height = 5)
